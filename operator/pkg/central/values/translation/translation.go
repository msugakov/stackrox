...
func translate(c platform.Central) (chartutil.Values, error) {
	...
	if features.PostgresDatastore.Enabled() {
		if c.Spec.CentralDB == nil {
			return nil, errors.Errorf("%s is enabled, but no CentralDB spec is specified", features.PostgresDatastore.EnvVar())
		}
		v.AddChild("centralDB", getCentralDBComponentValues(c.Spec.CentralDB))
	}
...
func getCentralDBComponentValues(c *platform.CentralDBComponentSpec) *translation.ValuesBuilder {
	cv := translation.NewValuesBuilder()
	...
	if c.ConnectionStringOverride != nil {
		cv.SetBoolValue("preexisting", true)
